...
def calc_nectar_humidity_factor(avg_humidity_pct):  # 定义函数：计算花蜜量湿度修正因子
    if avg_humidity_pct is None:  # 如果湿度为空
        ...
        return 0.90  # 返回一般值
    elif avg_humidity_pct <= 75:  # 如果湿度在 50%-75%
        return 1.00  # 返回最优值
    elif avg_humidity_pct <= 85:  # 如果湿度在 75%-85%
        return 0.88  # 返回偏湿值
    else:  # 如果湿度高于 85%
        return 0.72  # 返回高湿抑制值
...
def main():  # 定义主函数
    ...
    for row in flowering_rows:  # 遍历每一条花期结果
        ...
        env_modifier = (
            0.45 * temp_factor + 0.20 * humidity_factor + 0.35 * rain_factor
        )  # 计算花蜜量环境综合修正因子

        nectar_resource_modifier = 0.65 + 0.35 * clamp(nectar_resource_factor, 0.0, 1.1) / 1.1  # 把资源能力映射成较温和的修正值
